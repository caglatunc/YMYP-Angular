...
export class LoginComponent {
  ...
  signIn(form: NgForm) {
    if (form.valid) {
      this.http.post("https://localhost:7078/api/Auth/Login", { usernameOrEmail: form.controls["usernameOrEmail"].value, password: form.controls["password"].value })
        .subscribe((res: any) => {
      ...
      this.http.post("https://localhost:7078/api/ShoppingCarts/SetShoppingCartsFromLocalStorage", request).subscribe(res => {
